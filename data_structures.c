...
struct t_node *tree_init(void *k);
...
int t_add_node(void *k, struct t_node *tree_p);
...
int destroy_tree(struct t_node *tree_p);
...
int add_node(void *k, struct t_node *tree_p){
	struct t_node *p = malloc(sizeof(struct t_node));
	struct t_node *t = tree_p;
	if(!p) return 1;
